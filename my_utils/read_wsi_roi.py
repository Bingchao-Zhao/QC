...
def get_qupath_roi(roi_path, only_point=False):
    """
    返回的数组格式为：
    [polygon1, polygon2, ..., polygonn]
    polygon为一个numpy数组，大小为：(1, N, 2)，N表示点的个数，具体格式为：
    [[[a1,a2],[a3,a4], ..., [an,am]]]
    """
    ret_polygon = []
    ...
    if isinstance(data, dict):
        ret_polygon.append(np.array(data['geometry']['coordinates'][0],dtype=np.int))
    else:
        for polygon in data:
            ret_polygon.append(np.array(polygon['geometry']['coordinates'][0],dtype=np.int))
    return ret_polygon

def get_mask_from_qupath_point(ret_mask, rois):
    for i in rois:
        i = np.array([[ii[0], ii[1]] for ii in i],dtype=np.int)
        ret_mask = cv2.fillPoly(ret_mask, [i], (255))
